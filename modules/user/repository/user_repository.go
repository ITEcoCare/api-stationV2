...
func (r *repositoryUser) FindAll() response.RepositoryResult {
	...
	err := r.db.Preload("Role").Preload("Team").Find(&users).Error
	if err != nil {
		if err != nil {
			return response.RepositoryResult{
				Result: users,
...
func (r *repositoryUser) FindByIdWithRelation(Id int) response.RepositoryResult {
	...
	err := r.db.Preload("Role").Preload("Team").Where("id = ?", Id).First(&user).Error
	if err != nil {
		return response.RepositoryResult{Error: err}
	}
...
func (r *repositoryUser) FindAllWithTrashed() response.RepositoryResult {
	...
	err := r.db.Preload("Role").Preload("Team").Unscoped().Where("deleted_at is not null").Find(&users).Error
	if err != nil {
		return response.RepositoryResult{
			Result: users,
			Error:  err,
